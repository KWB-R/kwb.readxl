...
data_raw <- kwb.readxl::get_raw_text_from_xlsx(file, sheets = "two_empty_rows")

# Show the first rows of the first matrix
head(data_raw$sheet_01)

# Show the last rows of the first matrix
tail(data_raw$sheet_01)
```

The first empty columns are kept so that the row number in the returned matrix
...
```{r}
# Store the text matrix in x
x <- data_raw$sheet_01

# Get the number of the row containing a non-numeric value in column 2 
invalid_row <- max(which(is.na(as.numeric(x[, 2]))))
